...
// FOR CSV
  ...
  Future<void> saveDataToCsv(List<List<String>> data) async {
    ...
    final directory = await getExternalStorageDirectory();
    // YOU NEED TO CREATE A DIRECTORY FOR FILES NOT UPLOADED YET
    // BUT TO BE CREATED LATTER
    final file = File(
        '${directory!.path}/$deviceName$patientName$formattedDateTime$hour$minute$amOrPm.csv');
  ...
  // FOR CLOUD
  Future<void> saveCsvDataFilesNamesUploaded(String filePath) async {
    // This opens the file path
    // final file = await _externalFile;
    ...
    final directory = await getExternalStorageDirectory();
    Directory uploadDirectory = Directory('${directory!.path}/uploadDirectory');

    // CREATE THE DIRECTORY FOR FILES NOT UPLOADED YET HERE JUST FOR TESTING
    Directory recordedDataDirectory =
        Directory('${directory!.path}/recordedDataDirectory');
    if (!await recordedDataDirectory.exists()) {
      recordedDataDirectory.createSync(recursive: true);
    }
    ...
    // File name extracted here
    String extratedFileName = filePath.substring(86, 112);

    final uploadedFile = File('${uploadDirectory.path}/$extratedFileName.csv');

    if (await recordedFile.exists()) {
      final csvData = await recordedFile.readAsString();
      csvTable = const CsvToListConverter().convert(csvData);
    }
    if (!uploadDirectory.path.contains('$extratedFileName.csv')) {
      await uploadedFile.writeAsString(csvTable.toString());
    } else {
  ...
  // FOR CLOUD
  Future<List<List<dynamic>>> loadUpLoadData(String filePath) async {
    // final directory = await getApplicationDocumentsDirectory();
    final file = File(filePath);
