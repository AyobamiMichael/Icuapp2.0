...
class _PatientRecordedDataListScreenState
  ...
  void initState() {
    ...
    setState(() {
      _lengthOfFilesInDirectory =
          context.read<DirectoryProviderModel>().listOfFilesNames2.length;
    });
    print('length:$_lengthOfFilesInDirectory');
    progressHudTimer = Timer.periodic(Duration(milliseconds: 30), (timer) {
      generateCheckBox();
      setState(() {
        // _loading = true;
      });
  ...
  void generateCheckBox() {
    ...
    if (_directoryProviderModel.listOfFilesNames2.isNotEmpty) {
      checkBoxesList = List.generate(
          ...
          growable: true);
      //   print(_directoryProviderModel.listOfFilesNames2);
    } else {
